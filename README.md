...
- Throughout the repository, the name `saifa` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of saifa (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/saifa
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/saifa/saifa/ui_extension_example.py`.
